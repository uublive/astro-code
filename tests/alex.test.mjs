// Regression guard for commands/astro-alex.md — the "fast lane" command for long,
// unplanned prompts.  These tests pin the contracts that make astro-alex safe to hand
...
const src = readFileSync(join(COMMANDS, 'astro-alex.md'), 'utf8');

// ── 1. Raw prompt is captured verbatim as the source of truth ───────────────────────
test('astro-alex captures the raw prompt verbatim to PROMPT.md with a provenance marker', () => {
  ...
  assert.match(src, /<!--\s*astro-alex:\s*raw prompt\s*-->/, 'PROMPT.md needs the raw-prompt provenance marker');
});

// ── 2. Nothing is silently dropped ──────────────────────────────────────────────────
test('astro-alex parks anything it cannot classify in an explicit "To clarify" list', () => {
...
test('astro-alex requires each spec change to trace back to the raw prompt', () => {
...
test('astro-alex executes sequentially with no worktree fan-out', () => {
...
test('astro-alex skips the research/planning fan-out', () => {
...
test('astro-alex defaults the executor to Opus and honors --model override', () => {
...
test('astro-alex stamps commits with (phase NN tK) for resumable re-runs', () => {
...
test('astro-alex has a scope guard that escalates systemic changes to the full flow', () => {
...
test('astro-alex never auto-accepts — verified at best, human gate closes it', () => {
