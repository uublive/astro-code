...
test('templates/forge-knowledge.md names the read tool id', () => {
  assert.ok(
...
test('templates/forge-knowledge.md no longer contains the write tool or a WRITE protocol section', () => {
  assert.ok(
    !specSrc.includes(WRITE_TOOL),
    `templates/forge-knowledge.md must no longer contain "${WRITE_TOOL}" — astro-code no longer writes to forge (D1).`,
  );
  assert.ok(
    !/##\s*WRITE protocol/i.test(specSrc),
    `templates/forge-knowledge.md must no longer contain a "## WRITE protocol" section.`,
  );
});
...
test('the ToolSearch probe names exactly the two read tools the spec grants', () => {
  ...
  for (const tool of [READ_TOOL, LIST_TOOL]) {
    assert.ok(
  ...
  assert.ok(
    !probe[0].includes(WRITE_TOOL),
    `the ToolSearch probe must no longer name ${WRITE_TOOL} — astro-code no longer writes to forge (D1). Probe: ${probe[0]}`,
  );
});
...
//
// Table-driven over the read-side touched commands + touched agents: each must name
// `templates/forge-knowledge.md`, mention its relevant tool id in prose, and carry a
// skip-on-absence phrase. Deleting the pointer from any one file must go red. The three
// former write callers (astro-decision.md, astro-execute.md, astro-verify.md) no longer
// touch forge at all — see the section 3 grant-split tests below.
...
test('no commands/ or agents/ file grants or names mcp__forge__forge_capture_knowledge', () => {
  const offenders = [];
  for (const f of allCommandFiles) {
    const src = readFileSync(join(COMMANDS_DIR, f), 'utf8');
    if (src.includes(WRITE_TOOL)) offenders.push(`commands/${f}`);
  }
  for (const f of allAgentFiles) {
    const src = readFileSync(join(AGENTS_DIR, f), 'utf8');
    if (src.includes(WRITE_TOOL)) offenders.push(`agents/${f}`);
  }
  assert.deepEqual(
    offenders,
    [],
    `no commands/ or agents/ file may grant or name "${WRITE_TOOL}" — astro-code no longer writes ` +
      `to forge (D1). Offenders: ${offenders.join(', ') || 'none'}.`,
  );
});
