...
## Invocation
    ...
    python3 tools/kit_run_local.py --astro-root ~/Development/astro --run \
...
## Exit codes
    ...
    0  the kit resolved and loaded (and, with --run, every declared phase
       output was produced)
...
def main() -> int:
    ...
    ap.add_argument("--task", default="",
                    help="the prompt to run (required with --run)")
    ...
    if args.run and not args.task:
        die("--run needs --task \"<prompt>\" (try the Quick Start prompt from src/EXAMPLES.md)")
    ...
    try:
        ...
        if not args.run:
            ...
            print('        --run --task "<the Quick Start prompt from src/EXAMPLES.md>"')
            print()
