#!/usr/bin/env node
// SessionStart hook — the "there's an update" banner, exactly like GSD's.
// Two responsibilities, both cheap and non-blocking:
//   (1) read the cached check and, if the clone is behind origin, print a
//       systemMessage so the user sees it at the top of the session;
//   (2) if the cache is missing or stale (>1h), spawn the detached worker to
//       refresh it for next time (a `git fetch` + behind-count in the background).
// The one-session lag between (2) writing and (1) reading mirrors GSD by design —
...
if (cache && cache.update_available) {
  ...
  const msg = `astro-code update available: ${n} commit${n === 1 ? '' : 's'} behind${ver} — run /astro-update`;
