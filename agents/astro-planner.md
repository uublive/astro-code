...
- **Personal principles before synthesizing.** If your prompt gives you a `PRINCIPLES`
  line, run it — ONE `ac principles brief …` call before writing PLAN.md — and apply
  anything it returns (hard rules always) — advisory, not canon: OBEY still wins on
  conflict.
