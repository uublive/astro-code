...
## ADR-052 — The fixture-currency advisory check is its own ac verb in ac preflight's posture (exit 0 always, silent when clean), wired into /astro-execute after the waves AND into /astro-fast, scoped to the phase's own ADR-017 stamped commits. Data-model paths and seed source are declared in RUN-CONTRACT.md; a project with no declaration is reported as 'not checked', never as clean. Findings are both reported in the run summary and filed as debt. The criteria-author gets a standing, outcome-shaped rule keyed on the goal implying persisted state.
_2026-09-19_

**Why:** The check must fire without anyone remembering, so it takes the slot ADR-041's stamp audit already occupies, where the diff exists and the phase is not yet verified. /astro-fast is the lane ADR-050 named as the reason this layer exists at all — it has no criteria-author, so omitting it there leaves exactly the gap. Stamped commits scope the diff to THIS phase, where branch-versus-base would blame the last phase to run for an earlier one's schema change. A missing declaration must be distinguishable from a clean result, because a project that never opted in looking identical to one that passed is this phase's own failure mode in miniature. The criteria-author is plan-blind and pre-implementation, so it cannot read a diff and must infer from the goal it already reads.

## ADR-053 — Canon sync is refuse-first and content-identified: pull REFUSES to overwrite a diverged CONVENTIONS.md (naming push or an explicit force) instead of merging prose; a decision is identified by normalized CONTENT excluding its date stamp and tolerating dash variants, with strict equality only and never similarity; a genuine same-id-different-content collision REFUSES and reports both rather than renumbering anything; editing a published decision is refused with supersession explained; ac decision add publishes CONVENTIONS.md too; duplicates are detected always but collapsed only on explicit request; and pull reports per file what it changed versus left alone.
_2026-09-19_

**Why:** The canon is read by every planner, executor and verifier before it acts, so corruption is silent and total. Prose three-way merge was rejected despite being possible: conflict markers in the one file agents read as canon would be read as rules. Refusing to renumber removes the failure mode instead of reporting it, because decision numbers are cited by other files and a moved number silently points at nothing. Content identity excluding the date stamp is what lets the same decision recorded on two machines converge rather than collide. Auto-collapse was rejected on a precedent already set elsewhere: an automatic similarity-driven merge was deliberately removed because destructive wrong merges outweighed the convenience, leaving only exact-normalized auto-merge. The immutability rule that projects currently rediscover by corrupting their own canon becomes something the tool teaches and enforces.

## ADR-054 — Fixture-currency specifics this plan pinned: `ac fixtures check [--phase <N>]` as the sole verb/flag; a live-but-empty `<!-- astro-code: fixtures-declaration -->` marker block with `data-model:`/`seed:` keys in `RUN-CONTRACT.md`; stdout-only output using `⚠` (fired) and `⊡` (not checked), with silence as the clean signal; and a phase-invariant debt title
_2026-09-19_

**Why:** ADR-050/052 settled the three layers (canon rule, behavioural criterion, advisory `ac` verb) but deliberately left the verb name, the declaration format and the output shape open; phase 17's plan pins them once here so the doc task, the engine, both scaffolding commands and both rehearsals converge on identical values instead of each inventing its own. Recording the reasoning here, not only in the plan's pinned-values table, keeps it findable once the plan itself is stale.

**Rejected:** a placeholder example declaration shipped live in the template (an un-filled copy would read as declared, i.e. clean — this phase's own failure mode); glob or heuristic inference of data-model paths (D5 — a wrong guess is worse than an honest "not checked"); the phase number or a SHA in the debt title (breaks `dedupeKey`, turning a repeat sighting into a second item); and filing the finding from command prose instead of the verb itself (duplicates the logic across two lanes that already differ in structure).

## ADR-055 — Human-facing agent output leads with the change and keeps evidence beneath it; machine-read artifacts (PLAN.md, CRITERIA.md) stay dense. The verifier keeps writing full evidence and the COMMAND summarises it. Every human-facing reporting slot states a line budget or a silence rule, asserted by a guard test on slot shape; free-form narration gets a canon rule that is explicitly labelled unenforced. The Voice rule ships to generated projects via templates/CONVENTIONS.md.
_2026-09-20_
...
## ADR-061 — Principle review specifics: exact-equality dedupe, surfaced-only overlap, append-only sightings, merged terminal state, sync unions sightings, reopen needs a reason
_2026-09-24 · at c42e814_

**Why:** Phase 24 pinned the exact mechanics before wave 2 implemented them: (1) exact = normalised string equality only, never a Jaccard/embedding score — a similarity-driven auto-merge destroyed data before (ADR-053 precedent); (2) overlap = at least 2 shared non-stopword tokens (plural-folded), surfaced via ac principles match and never acted on automatically — the capturing agent or a human decides; (3) sighting: is a new repeatable header key, append-only, rendered after history and kept OUT of it so an accepted/rejected entry's history stays byte-identical after a sighting, and so two machines can append offline and merge with no conflict; sightingCount counts only the repeats, never the founding capture; (4) the parser now enforces HEADER_KEYS order as damage, closing a gap ADR-059 always claimed but never checked; (5) merged/merged-into is a new terminal status mirroring superseded/superseded-by (its own key, not an overload), and merge always writes the survivor first so a crash between the two writes can only duplicate evidence, never lose it; (6) sync's resolveMergeConflicts calls reconcileRevisions, which strips sightings, defers to the unchanged compareRevisions for the lifecycle, then unions sightings back in on any non-conflict outcome; (7) reopen (the only way back from rejected) requires a reason, exactly like reject/retire/amend.

**Rejected:** Reusing lib/registry.mjs's classifyMatch (whole-statement Jaccard >= 0.5) was rejected: it misses C4's case where two statements share only 2 of 8 content words yet are unmistakably the same idea, and it cannot explain which words matched. A float overlap score was rejected for the same reason a score was rejected elsewhere in this codebase (ADR-053): no threshold to tune, no explanation, no deps. Auto-merging on overlap was rejected — D1 requires a human or agent judgement, never a silent merge on similarity. A plain sighting COUNTER field was rejected: it cannot be sync-merged the way an array of records with a deterministic union can. A history line per sighting was rejected: it would make an accepted/rejected entry's history diverge on every repeat, breaking C2's byte-identical-history guarantee. Per-entry evidence caps were rejected: capping would mean rewriting or dropping OLD sighting lines, breaking the append-only property two machines' syncs depend on. Reusing reject or superseded-by for a duplicate was rejected: a rejection means 'no', a duplicate means 'yes, but keep the other wording' — conflating them would make a later exact re-proposal of the duplicate's wording land as a rejection instead of a sighting on the survivor.

## ADR-062 — Retrieval specifics phase 25 pinned: structural scope match (AND across stack/work/files dimensions, OR within one, rules bypass scope, only accepted entries served); `ac principles brief/ask/cite` never run principlesSync (hot path of every task/session start); stack detection is manifest-presence only with an `.astrocode/config.json` override that REPLACES detection; canon-clash is a keyword-overlap candidate only (phase 24's matcher reused, never forked), promotion-into-this-project suppresses it, canon always wins; the usage log lives at `<store>/.local/usage.jsonl`, ids only, per-machine and unsynced; the verifier sees hard rules only and a violation is a non-blocking finding, never a criterion failure; every forge read call site (D3 of this phase's CONTEXT.md) is replaced by one `ac principles ask`/`brief` call
_2026-09-24 · at 44366a3_

**Why:** phases 25's plan pinned these mechanics before wave 2+ implemented them, so a later change to lib/stack.mjs, lib/principlebrief.mjs, lib/principleask.mjs, lib/principlecanon.mjs, lib/principleusage.mjs or lib/retrieval.mjs has a canon line to check against. AND-across-dimensions (never OR) is what stops an "any tag overlaps anything" matcher from drowning a shortlist in noise. Never syncing on brief/ask/cite is what keeps a hook from ever waiting on a `git fetch`. The config override REPLACES rather than merges because a monorepo root's manifest can legitimately lie about a subproject's stack, and merging would make that lie unfixable. Canon-clash stays a candidate, never a resolution, for the same reason ADR-053/058 already settled elsewhere: an automatic merge on similarity is destructive and has already caused data loss in this codebase. The usage log is per-machine (`.local/`, already gitignored) because an append-only file is not merge-safe under git without phase-24-style reconciliation, and building that twice was rejected as premature. The verifier stays non-blocking on personal rules because CRITERIA.md is the sole pass/fail bar (ADR-021) and a personal preference must never gate a phase.

**Rejected:** Syncing brief/ask/cite like every other principles verb (rejected — the hot-path cost is the whole reason this phase exists, and an open conflict still surfaces on stderr without blocking); merging the config stack override into detection instead of replacing it (rejected — an unfixable lie is worse than an override that occasionally over-corrects); resolving canon-clash automatically by whichever text is "newer" (rejected — D5 requires an agent or human judgement, canon always wins, nothing is silently reconciled); syncing the usage log now (deferred on purpose — phase-24-style reconciliation for an append-only file is real work with no forcing function yet); embeddings for `ask` (rejected outright by CONTEXT — no external service, no embeddings, ever, in `lib/`).

## ADR-063 — Transcript-miner specifics this plan pinned: read-only scan with an explicit --advance commit step; the per-machine watermark lives under the store's gitignored .local/mine/ holding only pointers and hashes, never transcript text; a chunked reader with a 4 MiB per-line cap so one huge line can never force an unbounded read; per-host human-turn rules (Claude isMeta/isSidechain/origin/entrypoint, Codex payload.type/originator) with headless-session detection on both; one group-key function over token sets plus polarity (never a similarity score) with recurrence counted by distinct session, not raw repeats; an exact store match becomes a sighting and never a fresh candidate; MINE_CAP is guarded against the capture spec's own per-sweep number; the statusline/banner nudge is stat-only, Claude-only and never cached.
_2026-09-24 · at 7ba528e_
