...
export async function proposePrinciple(dir, {
  ...
  // Propose-time dedupe (D2): this read runs BEFORE the why check and before any
  // `mkdirSync`, so a fresh statement that turns out to need no dedupe and then fails
  // the why check below still leaves the store untouched (C2) — `loadPrinciples`
  // already tolerates an absent dir without creating it. `findCandidates` is run
  // against the REDACTED statement, matching what is already on disk (also redacted).
  ...
  // reviewer to judge it by, so it is refused before touching the filesystem — the
  // same "validate, then mkdir" ordering `addPrinciple`'s doc comment already promises
  // for kind/strength/work (C2). Scoped to a FRESH proposal only: a plain `add`
  // (propose:false, D7) is a human typing directly and is not subject to the review
  // rules, and a refresh (id present, just above) is re-stating an observation that
  // already carried one.
