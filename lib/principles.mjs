...
/**
 * A dated slug id for a principle statement (P5, ADR-013), ending in a 4-hex hash of
 * the normalised statement. The slug alone is cut at 40 characters, so two DIFFERENT
 * principles made on two machines could share an id — and then conflict, one taking the
 * other's place (phase-22 verify, C6/C10). The hash keeps them apart, and makes the SAME
 * statement captured on two machines converge on one id instead of conflicting.
 */
export function principleId(statement, now = new Date()) {
  const norm = String(statement || '').toLowerCase().replace(/\s+/g, ' ').trim();
  const hash = createHash('sha256').update(norm).digest('hex').slice(0, 4);
  return `${datedId(statement, now, 40, 'principle')}-${hash}`;
}
