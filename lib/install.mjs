...
function registerHooks(dir) {
  ...
  const stateCmd = (arg) => `"${node}" "${join(ASTRO_HOME, 'hooks', SESSION_STATE_HOOK)}" ${arg}`;
...
export function installClaude(frameworkRoot) {
  ...
  const linked = [];
  const home = resolve(frameworkRoot);
  for (const [dir, label] of configTargets()) {
    // Self-hosted guard: when a config dir IS the framework clone itself (this repo
    // dogfooding astro-code with CLAUDE_CONFIG_DIR pointed at the checkout), its
    // commands/ and agents/ ARE the tracked source files — symlinking the
    // ~/.astro/code copies over them would clobber real source with links (turning
    // `git status` into a wall of typechanges). Skip the symlinking there; the source
    // files are already where Claude Code discovers them. Hooks wiring stays (additive).
    const selfHosted = resolve(dir) === home;
    linked.push({
      label,
      dir,
      selfHosted,
      commands: selfHosted ? 0 : symlinkInto(join(ASTRO_HOME, 'commands'), join(dir, 'commands')),
      agents: selfHosted ? 0 : symlinkInto(join(ASTRO_HOME, 'agents'), join(dir, 'agents')),
...
export function installStatusline(frameworkRoot) {
  ...
  const wired = [];
  for (const [dir, label] of configTargets()) {
    wired.push({ dir, label, ok: registerHooks(dir) });
  }
  return { hooks, wired };
}
