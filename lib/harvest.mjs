...
import { classifyContext } from './planning.mjs';
...
// Delegates to `classifyContext` — the same classifier `ac phase context --author` uses —
// so "what counts as captured/agent-authored" stays defined in exactly one place
// (lib/planning.mjs).
function contextProvenance(file) {
  if (!existsSync(file)) return 'missing';
  return classifyContext(readFileSync(file, 'utf8')).kind;
}
...
//
// `sinceAdr` carries the previous milestone's `adr_watermark` (lib/milestone.mjs)
// alongside it — the finer, day-immune anchor `milestoneHarvest` prefers when it is
// available (an archive predating the watermark stamp has `null` here and the caller
// falls back to the date-only comparison it always did).
function resolveSince(root, n) {
  const prior = priorArchive(root, n);
  if (!prior) return { since: null, sinceAdr: null };
  const sinceAdr = Number.isInteger(prior.snapshot.adr_watermark) ? prior.snapshot.adr_watermark : null;
  if (prior.snapshot.closed_at) return { since: prior.snapshot.closed_at, sinceAdr };
  const accepted = (prior.snapshot.phases || [])
    .map((ph) => ph.accepted_at)
    .filter(Boolean)
    .sort();
  if (accepted.length) return { since: accepted[accepted.length - 1], sinceAdr };
  return { since: undefined, sinceAdr }; // neither timestamp — the "unknown" case
}
 ...
 */
export async function milestoneHarvest(root, n) {
  const p = paths(root);
  ...
  let source, phases, phaseDir, ownClosedAt, ownAdrWatermark;
  if (existsSync(archiveRoadmap)) {
    ...
    ownAdrWatermark = Number.isInteger(snapshot.adr_watermark) ? snapshot.adr_watermark : null;
  ...
  const { since, sinceAdr } = resolveSince(root, milestone);
  let adrWindow, adrs;
  if (since === undefined) {
    adrWindow = null;
    adrs = [];
    ...
    adrs = parseDecisionEntries(decisionsText)
      ...
      .map((e) => ({
        id: e.id,
        num: Number((e.id.match(/\d+/) || [])[0]),
        title: decisionTitle(e.text),
        date: decisionDate(e.text),
        why: decisionWhy(e.text),
      }))
      .filter((e) => e.date != null)
      // The watermark (lib/milestone.mjs `adr_watermark`) is the day-immune anchor: an
      // archive stamped with one is compared by ADR NUMBER, never by date, so two ADRs
      // opened on the same calendar day but on either side of the close never both land
      // in — or both fall out of — the SAME milestone's window (the leak this fixes).
      // Only an archive that predates the watermark stamp falls back to the date-only
      // comparison this module always did.
      .filter((e) => (sinceAdr != null ? e.num > sinceAdr : (sinceDay ? e.date >= sinceDay : true)))
      .filter((e) => (ownAdrWatermark != null ? e.num <= ownAdrWatermark : (untilDay ? e.date <= untilDay : true)))
      .map(({ num, ...rest }) => rest);
  }
