...
export function flowPR(root) {
  ...
  // We do NOT integration-test forge success (network + token required) — only the
  // detection/degrade logic is tested. When the CLI is absent, fall through to URL.
  if (prCfg === 'gh' && forgePresent('gh')) {
    // forge CLI would be invoked here; not integration-tested
  }
