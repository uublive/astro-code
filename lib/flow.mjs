...
//
// Reads the URL as CONFIGURED, not as `git remote get-url` reports it: get-url
// applies `url.<base>.insteadOf` rewrites, and those route transport (an internal
// mirror, https→ssh) — they are not where the forge's compare page lives. A
// github.com remote rewritten to a mirror must still link to github.com. Falls
// back to get-url for remotes defined outside config (legacy .git/remotes).
export function getRemoteUrl(remote, cwd) {
  const cfg = git(['config', '--get', `remote.${remote}.url`], { cwd })
...
export function flowBranch(root) {
  ...
  //
  // `ac init` also scaffolds AGENTS.md, for the same reason and at the same
  // moment, so an UNTRACKED one is excluded too. A MODIFIED AGENTS.md is not:
  // that file is user-owned outside astro-code's managed block, and letting a
  // real edit ride a branch silently is exactly what this guard is for.
  const porcelain = git(['status', '--porcelain'], { cwd })
  if (porcelain.stdout.trim()) {
    const scaffolded = (line) => {
      const path = line.replace(/^..\s+/, '')
      if (path.startsWith('.astrocode/')) return true
      return path === 'AGENTS.md' && line.startsWith('??')
    }
    const dirtyFiles = porcelain.stdout
      .trim()
      .split('\n')
      .map((l) => l.trim())
      .filter((l) => l && !scaffolded(l))
