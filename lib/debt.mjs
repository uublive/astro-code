 ...
 *   dropped  it stopped being true; the reason is recorded
 */
export const DEBT_STATUSES = ['open', 'paying', 'paid', 'dropped'];
...
export function loadDebt(root) {
  return readJSON(paths(root).debt) || { version: 1, debt: [] };
}
...
export async function addDebt(root, { title, why = '', phase = '', file = '', cost = 'small', now = new Date() } = {}) {
  ...
  return withLock(p.lock, () => {
    const db = readJSON(p.debt) || { version: 1, debt: [] };
...
export async function payDebt(root, ref, { kind, workRef, now = new Date() } = {}) {
  ...
  return withLock(p.lock, () => {
    ...
    if (entry.status === 'paid' || entry.status === 'dropped') {
