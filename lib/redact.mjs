...
// blocks, bare JWTs, `key = value` secret-labelled pairs and env-style `*_TOKEN=`-shaped
// assignments) is exactly the set phase 22's
...
const PATTERNS = [
  ...
  {
    re: /\b([A-Za-z0-9_]*(?:PASSWORD|PASSWD|PWD|SECRET|TOKEN|API_KEY|APIKEY|ACCESS_KEY|PRIVATE_KEY|CREDENTIAL)[A-Za-z0-9_]*)(\s*[:=]\s*)(?:"[^"\n]*"|'[^'\n]*'|\S+)/gi,
