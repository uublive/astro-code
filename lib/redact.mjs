...
const PATTERNS = [
  ...
  { re: /\bgl(?:pat|dt|ptt|rt)-[A-Za-z0-9_-]{16,}/g, replace: () => REDACTED },
  ...
  // `curl -u user:pw`, `--user=user:pw`, glued `-uuser:pw`, and a quoted `'user:pw two'`.
  { re: /(\bcurl\b[^\n]*?\s(?:-u|--user)(?:[\s=]+|(?=[^\s-])))(["'“‘]?)([^\s:"'”’]+):(?:[^"'”’\n]*["'”’]|\S+)/gi, replace: (_m, pre, q, user) => `${pre}${q}${user}:${REDACTED}` },
  { re: /(\bsshpass\b[^\n]*?\s-p\s*)(?:"[^"\n]*"|'[^'\n]*'|“[^”\n]*”|‘[^’\n]*’|\S+)/gi, replace: (_m, pre) => `${pre}${REDACTED}` },
  // `docker login -p X` / `--password X` (the flag rule above covers the long form).
  { re: /(\bdocker\s+login\b[^\n]*?\s-p\s*)(?:"[^"\n]*"|'[^'\n]*'|“[^”\n]*”|‘[^’\n]*’|\S+)/gi, replace: (_m, pre) => `${pre}${REDACTED}` },
