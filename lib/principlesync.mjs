...
//
// ## Why sightings union instead of conflicting (P4, D3)
//
// A genuine conflict is divergent history — two DECISIONS the human made differently on
// two machines, which is exactly what must never be silently resolved either way (see
// above). A sighting is not a decision, it is append-only EVIDENCE with no "winner": two
// machines each recording that the same principle recurred are not disagreeing about
// anything. `resolveMergeConflicts` therefore calls `reconcileRevisions`, which strips
// `sightings` before the UNCHANGED `compareRevisions` runs on the lifecycle fields, then
// unions both sides' sightings back in on any non-conflict outcome — same/older/newer all
// keep the winning lifecycle AND every sighting from both sides. `unionSightings`'s
// deterministic sort (by `at`, then the JSON string) is what makes two machines that each
// appended a sighting offline converge on a BYTE-IDENTICAL file, with no `conflicts/` file
// and no marker, even though neither side's history changed relative to the other's.
...
import { parsePrinciple, renderPrinciple, reconcileRevisions } from './principlemd.mjs';

// `.local/` holds per-machine state that must never sync — today, which side of each
// open conflict is THIS machine's (see `recordPerspectives`).
const GITIGNORE = '.lock/\n*.tmp-*\n.local/\n';

// The "dotfiles trap": `~/.astro/principles` sits inside SOME developers' `$HOME`, which
// is itself a git-tracked dotfiles repo (this is exactly why `isStoreRepo` checks `.git`
// presence and never `isRepo` — see the module header). Before this store has its OWN
// `.git` (or ever, if the user never runs `ac principles remote`), every entry written
// under it is an ordinary file on disk and would show up as untracked debris the moment
// `git status` runs in that ENCLOSING repo — silently pulling personal principles into
// view in a repo they were never meant to touch. Locally excluding the store's path via
// the enclosing repo's `.git/info/exclude` fixes that without writing to any TRACKED
// file (info/exclude lives inside `.git/`, invisible to `git status`, `git diff` and
// anyone who clones the dotfiles repo) — unlike a working-tree `.gitignore`, which would
// itself be a new change in that repo. A no-op when there is no enclosing repo, or when
// `dir` already IS one (own `.git`, so `relative()` comes back empty).
//
// Best-effort, and never able to fail a command (phase-22 verify, C1): the path comes
// from `git rev-parse --git-path info/exclude` — never a hand-built `<root>/.git/...`,
// which is wrong when `.git` is a FILE (a linked worktree or a submodule) — its parent
// is created when missing, and any error is swallowed: hiding the store from an
// enclosing repo is a courtesy, never a precondition.
function ignoreInEnclosingRepo(dir) {
  try {
    if (!existsSync(dir)) return;
    const top = git(['rev-parse', '--show-toplevel'], { cwd: dir });
    if (top.status !== 0) return;
    // Realpath BOTH sides: `--show-toplevel` is already resolved, `dir` may not be (a
    // store reached through a symlink — every macOS tmpdir, `/var` → `/private/var`).
    // Mixed, `relative()` came back `../…` and the store was never excluded
    // (phase 26 remediate-r2).
    const root = realpathSync(top.stdout.trim());
    const rel = relative(root, realpathSync(dir)).split('\\').join('/');
    if (!rel || rel.startsWith('..')) return;
    const gp = git(['rev-parse', '--git-path', 'info/exclude'], { cwd: root });
    if (gp.status !== 0 || !gp.stdout.trim()) return;
    const raw = gp.stdout.trim();
    const excludePath = isAbsolute(raw) ? raw : join(root, raw);
    const pattern = `/${rel}/`;
    let existing = '';
    try {
      existing = readFileSync(excludePath, 'utf8');
    } catch {
      // no info/exclude yet — start fresh below
    }
    if (existing.split('\n').some((l) => l.trim() === pattern)) return;
    mkdirSync(dirname(excludePath), { recursive: true });
    const sep = existing && !existing.endsWith('\n') ? '\n' : '';
    writeFileSync(excludePath, `${existing}${sep}${pattern}\n`);
  } catch {
    // best-effort (see above)
  }
}

// ── Which side of a conflict is THIS machine's (phase-22 verify, C12) ─────────
//
// "mine" used to be decided once, by the machine that FILED the conflict (its version
// stays in `<id>.md`), and every machine read it the same way. A machine that only
// PULLS that resolution sees the opposite: its own edit is now the conflict copy — yet
// it was told "this machine's version kept", and `resolve` (default mine) deleted that
// copy, i.e. its own amendment. So each machine works out its OWN perspective at the
// moment it files or pulls a conflict, and records it locally (`.local/`, never
// synced): 'entry' (my version is in <id>.md), 'copy' (my version is the conflict copy)
// or 'unknown' (neither is mine — e.g. a third machine). Nothing is ever resolved on a
// guess: `resolveConflict` refuses 'mine'/'theirs' while the perspective is unknown.
const perspectiveFile = (dir) => join(dir, '.local', 'conflicts.json');

function readPerspectives(dir) {
  try {
    return JSON.parse(readFileSync(perspectiveFile(dir), 'utf8')) || {};
  } catch {
    return {};
  }
}

function writePerspectives(dir, map) {
  mkdirSync(join(dir, '.local'), { recursive: true });
  atomicWriteText(perspectiveFile(dir), JSON.stringify(map, null, 2) + '\n');
}

// `filed`: the conflict files THIS sync's own merge created (ours kept → 'entry').
// `beforeHead`: this machine's commit before it pulled, to recognise its own version.
function recordPerspectives(dir, { filed = [], beforeHead = null } = {}) {
  const map = readPerspectives(dir);
  const open = listConflictFiles(dir);
  const openFiles = new Set(open.map((c) => c.file));
  for (const f of Object.keys(map)) if (!openFiles.has(f)) delete map[f];
  for (const c of open) {
    if (map[c.file]) continue;
    if (filed.some((x) => x.file === c.file)) { map[c.file] = 'entry'; continue; }
    let before = null;
    if (beforeHead) {
      const shown = git(['show', `${beforeHead}:${c.id}.md`], { cwd: dir });
      if (shown.status === 0) before = shown.stdout;
    }
    const read = (p) => { try { return readFileSync(join(dir, p), 'utf8'); } catch { return null; } };
    const copy = read(c.file);
    const entry = read(`${c.id}.md`);
    map[c.file] = before != null && before === copy ? 'copy' : before != null && before === entry ? 'entry' : 'unknown';
  }
  writePerspectives(dir, map);
}
...
function resolveMergeConflicts(dir) {
  ...
  for (const line of unmerged) {
    ...
    // 'UU' is the ordinary both-modified conflict; 'AA' is both-added — two machines
    // minting the SAME id offline (D4's "nothing to coordinate" collides here, the one
    // place two independent entries can land on one path). `ls-files -u` reports stage
    // 2/3 for both shapes identically, so both route through the same genuine-conflict
    // handling below. Any other code (DD/AU/UA/UD/DU — a delete on one side) is not ours
    // to guess at.
    if ((code !== 'UU' && code !== 'AA') || !/^[^/]+\.md$/.test(path)) return { ok: false, conflicts: [] };
    ...
    // `reconcileRevisions` strips sightings, defers to the UNCHANGED `compareRevisions`
    // for the lifecycle fields, then unions the sightings back in on any non-conflict
    // outcome (P4, D3) — see the module header paragraph below for why.
    const reconciled = reconcileRevisions(oursEntry, theirsEntry); // ours relative to theirs
    if (reconciled.outcome !== 'conflict') {
      atomicWriteText(join(dir, path), renderPrinciple(reconciled.entry));
...
async function syncLocked(dir) {
  ignoreInEnclosingRepo(dir);
  if (!isStoreRepo(dir)) return EMPTY();
  if (!getRemote(dir)) return EMPTY();
  ensureGitignore(dir); // stores created before `.local/` existed pick it up here
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    if (!hasOriginMain) {
      ...
      if (push.status === 0) return { state: 'synced', pulled, pushed: true, conflicts: openConflicts(dir) };
    ...
    recordPerspectives(dir, { filed: conflicts, beforeHead });
    ...
    // `conflicts` here is only what THIS call's own merge resolved — a fast-forward pull
    // (this machine has no divergent commit of its own) never runs resolveMergeConflicts
    // at all, yet can still land files under `conflicts/` that a DIFFERENT machine filed
    // and already pushed. Re-reading openConflicts(dir) reports every conflict that is
    // open on disk after this sync, not just the ones this call happened to resolve, so
    // a caller that only checks the return value (not every command re-reading disk)
    // still sees it.
    if (push.status === 0) return { state: 'synced', pulled, pushed: true, conflicts: openConflicts(dir) };
  }
  ...
  return { state: 'synced', pulled, pushed: false, conflicts: openConflicts(dir) };
}
...
function listConflictFiles(dir) {
...
/**
 * Every entry with an unresolved sync conflict: `id`, `file` (the conflict copy), and
 * `mine` — which side is THIS machine's: 'entry' (in `<id>.md`), 'copy' (the conflict
 * copy) or 'unknown'.
 */
export function openConflicts(dir) {
  const map = readPerspectives(dir);
  return listConflictFiles(dir).map((c) => ({ ...c, mine: map[c.file] || 'unknown' }));
}
...
export async function resolveConflict(dir, id, { take = 'mine' } = {}) {
  if (!['mine', 'theirs', 'entry', 'copy'].includes(take)) throw new Error(`invalid take: ${JSON.stringify(take)}`);
  return withLock(join(dir, '.lock'), async () => {
    const mine = openConflicts(dir).filter((c) => c.id === id);
    if (mine.length === 0) throw new Error(`no open conflict for ${id}`);

    // mine/theirs are relative to THIS machine; entry/copy name the files outright.
    let side = take;
    if (take === 'mine' || take === 'theirs') {
      const p = mine[0].mine;
      if (p === 'unknown') {
        throw new Error(
          `cannot tell which version of ${id} is this machine's — choose explicitly: ` +
            `--take entry (keep ${id}.md) or --take copy (keep ${mine[0].file}); nothing was changed`,
        );
      }
      side = take === 'mine' ? p : p === 'entry' ? 'copy' : 'entry';
    }
    ...
    const map = readPerspectives(dir);
    for (const c of mine) delete map[c.file];
    writePerspectives(dir, map);
    return { kept: side };
  });
}
