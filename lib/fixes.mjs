...
/**
 * Build a dated slug id from a title. Truncated on a WORD boundary so a long
 * report does not produce a 200-character directory name, and never left with
 * a trailing hyphen.
 *
 * Shared with the debt register (`lib/debt.mjs`): both are unplanned, name-identified
 * objects under ADR-013, and one id scheme in both places is what keeps `ls`, the
 * registry and the archive sorting the same way without extra machinery.
 */
export function datedId(title, now = new Date(), maxSlug = 40, fallback = 'fix') {
  ...
  return `${today(now)}-${short || fallback}`;
}

/** Build a fix id from a title. */
export function fixId(title, now = new Date(), maxSlug = 40) {
  return datedId(title, now, maxSlug, 'fix');
}
