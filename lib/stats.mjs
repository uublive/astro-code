...
// Claude Code keys a project's transcripts by its path with '/' and '.' → '-'
// (e.g. /home/user.name/proj → -home-user-name-proj).
export function transcriptDir(root) {
  return join(configDir(), 'projects', root.replaceAll('/', '-').replaceAll('.', '-'));
}
