...
import { parseDecisions, parseDecisionEntries, sameDecision, findDuplicates, collapseDuplicates } from './decisions.mjs';
...
export function canonDedupe(root) {
  ...
  if (where === 'present') {
    const res = transact(root, { remote, branch, message: 'canon: collapse duplicate decisions' }, (files) => {
      ...
      const { text, removed } = collapseDuplicates(files[DECISIONS_FILE]);
  ...
  if (removed.length) atomicWriteText(p.decisions, text);
