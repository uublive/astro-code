...
function entryText(id, { title, why, rejected, when, commit }) {
  return (
...
function buildDecision(existing, { title, why, rejected, date, commit }) {
  ...
  const entry = entryText(id, { title, why, rejected, when, commit });
  return { id, when, next: (existing.trimEnd() + '\n' + entry).trimStart() };
}

// #77 — the decision being added, if the log already holds it IN FORCE. Identity is the
// same normalized equality dedupe uses (id, date and commit stamp excluded), so a retried
// `decision add` finds its first attempt. A retired or superseded twin does not count:
// re-adding its text is re-adopting it, which is a new decision.
function liveTwin(text, { title, why, rejected }) {
  const candidate = entryText('ADR-000', { title, why, rejected, when: '1970-01-01' }).trim();
  return parseDecisionEntries(text).find((e) => sameDecision(e.text, candidate) && decisionStatus(e.text).state === 'live') || null;
}
...
export async function addDecision(root, { title, why = '', rejected = '', date } = {}) {
  ...
  if (isRepo(root) && hasRemote(remote, root)) {
    ...
    const res = transact(root, { remote, branch, message: `canon: decision "${title}"` }, (files) => {
      ...
      twin = liveTwin(merged.text, { title, why, rejected });
      if (twin) return { updates: {}, result: twin.id };
    ...
    if (res.ok && built) {
      ...
      return {
        ...
        // #77 — over the log AFTER the append, so the add that creates a duplicate is the
        // one that reports it (the merge's own list predates the new entry).
        duplicates: findDuplicates(built.next),
        publishedConventions,
        conventionsRefused,
      };
    }
    if (res.ok && twin) {
      return { id: twin.id, title, existing: true, source: 'remote', branch, preserved: [], duplicates: [] };
    }
    if (res.ok && !built) {
  ...
  return withLock(p.lock, () => {
    ...
    const same = liveTwin(existing, { title, why, rejected });
    if (same) return { id: same.id, title, existing: true, source: 'local', duplicates: [] };
    const { id, when, next } = buildDecision(existing, { title, why, rejected, date, commit });
    writeDecisions(p, next);
    return { id, title, date: when, source: 'local', duplicates: findDuplicates(next) };
  });
}
...
//
// #75 — every entry is compared, not the first under each id. The first-wins
// `parseDecisions` is lossy by construction, and this is the one caller that must not
// lose an entry: a mirror holding a second ADR-001 is not the registry's copy. The
// preamble above the first heading is still not compared — it is not a decision.
export function canonDrift({ localDecisions = '', registryDecisions = '', localConventions = null, registryConventions = null }) {
  ...
  const loc = byId(localDecisions);
  const reg = byId(registryDecisions);
  for (const [id, entries] of loc) {
    const theirs = reg.get(id);
    if (!theirs) drift.push({ id, kind: 'missing from the registry' });
    else if (entries.length > theirs.length) drift.push({ id, kind: `repeated id locally (${entries.length} entries, the registry has ${theirs.length})` });
    else if (entries.length < theirs.length) drift.push({ id, kind: `repeated id in the registry (${theirs.length} entries, local has ${entries.length})` });
    else if (entries.length === 1 && entries[0] !== theirs[0]) drift.push({ id, kind: isOlderRevision(entries[0], theirs[0]) ? 'older revision locally (pull)' : 'changed body' });
    else if (entries.some((e, i) => e !== theirs[i])) drift.push({ id, kind: 'changed body' });
  }
