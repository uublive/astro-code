...
// Polarity buckets, not a lexeme set (C4 remediation): "no", "don't"/"dont" and "not" are
// all the SAME negative instruction spoken three different ways — collapsing them into
// one canonical tag is what lets "No, don't add semicolons…" / "Don't add semicolons…" /
// "Do not add semicolons…" group as one steer. Only which BUCKET is present matters, never
// how many of its words fired or in what combination, so a redundant "No," prefix in front
// of "never" can no longer split it from a bare "never …" (both are the 'neg' bucket).
// "always"/"sempre" stay a separate 'pos' bucket so "always run the linter" and "never run
// the linter" remain opposite instructions (module header, ADR-053).
const POLARITY_GROUPS = [
  { tag: 'neg', words: ['not', 'no', 'never', 'don', 'dont', 'nor', 'non', 'mai'] },
  { tag: 'pos', words: ['always', 'sempre'] },
];
...
// Contractions are expanded BEFORE keying (C4, second verify): "don't", "dont" and "do not"
// — and "shouldn't"/"should not" — are one instruction, but only `don` and `t` were
// stopwords, so "dont" survived as a content word and split a real repeat into two
// one-off groups. After expansion every form reads "<aux> not", and both halves are
// filler to the key.
const AUX_NOT = /\b(do|does|did|should|would|could|is|are|was|were|has|have|had|must|need)(?:n't|nt)\b/g;
const KEY_FILLER = new Set(['did', 'has', 'have', 'had', 'could', 'ca', 'wo']);

function expandNegations(s) {
  return String(s ?? '')
    .toLowerCase()
    .replace(/[’`]/g, "'")
    .replace(/\bcan'?t\b/g, 'can not')
    .replace(/\bwon'?t\b/g, 'will not')
    .replace(AUX_NOT, '$1 not');
}

function keyTokens(text) {
  return statementTokens(text).filter((t) => !MINE_FILLER.includes(t) && !KEY_FILLER.has(t));
}

// A contrast names a PREFERRED side and an AVOIDED side, and the key must keep them
// apart: as an unordered token set, "use tabs not spaces" and "use spaces not tabs" were
// the same key, so two opposite one-off corrections merged into one steer with a false
// recurrence of 2 (C4, second verify). Three shapes are recognised, clause by clause:
//   - in-clause "X not|never|rather than|instead of Y", with content on both sides;
//   - a clause OPENING with "not|rather than|instead of" after a clause with content
//     ("use tabs, not spaces");
//   - an "always …" clause alongside a "never …/don't …" clause.
// Anything else keeps the unordered set + polarity, so "in tests, never mock the database"
// still groups with "never mock the database in tests".
const CONTRAST_MID = [['not'], ['never'], ['rather', 'than'], ['instead', 'of']];
const CONTRAST_LEAD = [['not'], ['rather', 'than'], ['instead', 'of']];
const POS_WORDS = POLARITY_GROUPS.find((g) => g.tag === 'pos').words;
const NEG_WORDS = POLARITY_GROUPS.find((g) => g.tag === 'neg').words;

function findMarker(words, markers, from = 0) {
  for (let i = from; i < words.length; i++) {
    for (const m of markers) {
      if (m.every((w, k) => words[i + k] === w)) return { at: i, len: m.length };
    }
  }
  return null;
}

function contrastSides(expanded) {
  const clauses = expanded.split(/[,;:]|\s[-–—]\s|\bbut\b/).map((c) => normForCue(c)).filter(Boolean);
  const pref = new Set();
  const avoid = new Set();
  let sided = false;
  let hasPos = false;
  ...
  for (const c of clauses) {
    ...
    const lead = words[0] === 'and' ? 1 : 0;
    const leadHit = findMarker(words.slice(lead, lead + 2), CONTRAST_LEAD);
    if (leadHit && leadHit.at === 0 && seenContent) {
      toks.forEach((t) => avoid.add(t));
      sided = true;
    } else {
      const mid = findMarker(words, CONTRAST_MID);
      const before = mid ? keyTokens(words.slice(0, mid.at).join(' ')) : [];
      const after = mid ? keyTokens(words.slice(mid.at + mid.len).join(' ')) : [];
      if (mid && before.length && after.length) {
        before.forEach((t) => pref.add(t));
        after.forEach((t) => avoid.add(t));
        sided = true;
      } else {
        clauseInfo.push({ toks, pos: hasPhrase(c, POS_WORDS), neg: hasPhrase(c, NEG_WORDS) });
      }
    }
    if (toks.length) seenContent = true;
  }
  for (const ci of clauseInfo) { hasPos = hasPos || ci.pos; hasNeg = hasNeg || ci.neg; }
  if (!sided && !(hasPos && hasNeg)) return null;
  for (const ci of clauseInfo) {
    const side = ci.neg && !ci.pos ? avoid : pref;
    ci.toks.forEach((t) => side.add(t));
  }
  return { pref: [...pref].sort(), avoid: [...avoid].sort() };
}

/** The one group-key function (ADR-053): token set (minus filler) + polarity, or a sided contrast, hashed. */
export function steerKey(sentence) {
  const expanded = expandNegations(sentence);
  const sides = contrastSides(expanded);
  let key;
  if (sides && (sides.pref.length || sides.avoid.length)) {
    key = `prefer:${sides.pref.join(' ')}|avoid:${sides.avoid.join(' ')}`;
  } else {
    const norm = normForCue(expanded);
    const pol = POLARITY_GROUPS.filter((g) => hasPhrase(norm, g.words)).map((g) => g.tag);
    const toks = keyTokens(expanded);
    // A short steer (e.g. "never use tabs for indentation") can fall under 3 content
    // tokens; falling all the way back to the raw normalised SENTENCE there (as opposed to
    // its filler-stripped token set) let a leading "No," or a trailing "please" split it
    // from an otherwise-identical restatement (C4 remediation) — so the fallback is only
    // for the case the token set itself is empty (every word was a stopword/filler).
    key = toks.length ? `${toks.join(' ')}|${pol.join(' ')}` : normaliseStatement(sentence);
  }
