...
//   - an EXACT-normalised collapse (`normaliseStatement`, the same equality phase 24's
//     store dedupe uses): byte-identical-after-normalising turns become one item carrying
//     the union of their sessions, because handing the agent the same "No." forty times
//     is noise, not evidence — the session list keeps the evidence;
...
import { normaliseStatement, findCandidates, pickExactTarget } from './principlematch.mjs';
...
/**
 * Collapse occurrences whose redacted text is identical after `normaliseStatement` —
...
function collapse(occurrences) {
  ...
  for (const occ of occurrences) {
    const key = normaliseStatement(occ.text) || occ.text.trim();
...
export async function sweep({ scope = { mode: 'project', roots: [] }, rescan = false, storeDir, env = process.env } = {}) {
  ...
  for (const item of collapsed) {
    ...
    const { exact } = findCandidates(entries, latest.text);
