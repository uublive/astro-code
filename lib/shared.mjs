...
export function readTree(root, tip) {
  ...
  // --full-tree: from a project rooted in a subdirectory of the repo, plain ls-tree lists
  // only that subpath, and the branch's root-level files read as absent.
  const res = git(['ls-tree', '--full-tree', tip], { cwd: root });
...
export function transact(root, { remote, branch, retries = 6, message = 'update' }, fn) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    ...
    // Nothing to write → write nothing. Committing an unchanged tree still pushed an empty
    // commit onto the shared branch (a refused `decision add` did exactly that), and a
    // caller that finds nothing to change — a dedupe with no registry-side duplicate, a
    // repoint that is already in place — must not move the tip everyone fetches.
    if (Object.keys(updates).length === 0) {
      return { ok: true, result, attempt, branch, owner: id.owner, noop: true };
    }
