...
//
// Anchored to the START of the file (optional leading whitespace only) rather than
// searched anywhere in the text: the marker is stamped as the FIRST line by /astro-discuss
// (see CONTEXT_MARKER above), so a human CONTEXT.md that merely quotes the agent form in
// its prose — to document ADR-037's two forms, say — must not be misread as agent-authored.
// Un-anchored, `ac milestone harvest` dropped a genuine human capture from its sweep.
export function contextAuthor(text) {
  const m = String(text).match(/^\s*<!--\s*astro-discuss:\s*captured\s+by\s+agent:\s*([^>]*?)\s*-->/i);
