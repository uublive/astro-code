...
/**
 * Every run of sentence punctuation / whitespace / dashes / quotes collapses to one space;
 * case and Unicode forms fold. Operator-like symbols (`+ = < > # & | ~ ^ * %`) are KEPT as
 * part of the word: stripping them made "Prefer C over C++" equal "Prefer C++ over C" and
 * "use === not ==" equal "use == not ===", so a steer was recorded as a sighting of its
 * own OPPOSITE — even of a rejected entry (phase 26 verify, C4). Sentence punctuation
 * (`. , ; : ! ?`), quotes, brackets and dash variants still fold away, which is the
 * phase-24 C1 guarantee.
 */
