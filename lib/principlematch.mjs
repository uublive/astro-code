 ...
 * the TOKENISING form (`statementTokens`, overlap candidates), where symbols carry no
 * content words. Exact equality does NOT use it — see `exactKey`.
 */
...
// Exact equality folds only what never carries meaning: case, Unicode forms, whitespace,
// sentence punctuation at the END of a word, quotes/brackets AROUND a word, and words that
// are nothing but dashes or sentence punctuation. Symbols INSIDE a word are kept. Folding
// every symbol made a steer equal its own opposite — "C++ over C" vs "C over C++",
// `:=` vs `=`, `$(cmd)` vs `` `cmd` ``, `i--` vs `--i`, `?.` vs `.` — so it was recorded as
// a sighting of a REJECTED entry and never reached the agent (phase 26 verify, C4). A word
// that is only a quote or an operator stays as itself (`'` vs `"` are different advice).
// Phase 24's C1 still holds: case, `. , ; : ! ? …`, whitespace and dash variants fold.
const DASH_WORD = /^[-‐‑‒–—―]+$/u;
const PUNCT_WORD = /^[.,;:!?…]+$/u;
const LEAD = /^["'`“”‘’«»([{]+/u;
const TRAIL = /[.,;:!?…"'`“”‘’«»)\]}]+$/u;

/** The key two statements must share to be the SAME statement (`sameStatement`). */
export function exactKey(s) {
  const words = String(s ?? '').normalize('NFKC').toLowerCase().split(/\s+/u).filter(Boolean);
  const out = [];
  for (const w of words) {
    if (DASH_WORD.test(w) || PUNCT_WORD.test(w)) continue;
    const core = w.replace(LEAD, '').replace(TRAIL, '');
    out.push(core || w);
  }
  return out.join(' ');
}
