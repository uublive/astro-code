...
// #37 — a milestone claim can be `planned` (declared as a destination, not yet the current
// work) as well as `active`/`complete`. `number` claims an EXPLICIT milestone number: only
// the guarded repair uses it (`ac milestone new --planned --number N`), and it refuses a
// number that is already claimed — it can ratify a state the roadmap is in, never mint over
// an existing claim.
export function claim({ root, type, milestone, name = '', remote, branch, retries = 6, status = 'active', number: explicit }) {
  remote = remote || registryRemote(root);
  branch = branch || registryBranch(root);
  ...
  const res = transact(root, { remote, branch, retries, message }, (files, id) => {
    ...
    if (explicit != null && registry.claims.some((c) => c.type === type && c.number === explicit)) {
      return { updates: {}, result: { taken: explicit } };
    }
    const number = explicit != null ? explicit : nextNumber(registry, type, floor[type]);
    ...
    if (type === 'milestone') {
      ...
      registry.claims.push({ type: 'milestone', number, name, owner: id.owner, branch: id.branch, claimed_at, status });
    } else {
  ...
  if (res.ok && res.result && res.result.taken != null) {
    return { number: null, source: 'error', error: `${type} ${res.result.taken} is already claimed on ${branch}` };
  }
  if (res.ok) {
    return {
...
// #37 — move a planned milestone to active. Refuses an unclaimed number or a closed one;
// a claim that is already `active` succeeds as a no-op (`already: true`) — that is how a
// milestone claimed ahead as active (#76) is moved into by the CLI, which switches the
// project to it either way.
export function activateMilestone({ root, number, remote, branch, retries = 6 }) {
  remote = remote || registryRemote(root);
  branch = branch || registryBranch(root);
  const where = probeBranch(root, { remote, branch });
  if (where === 'unreachable') return { ok: false, error: `cannot reach \`${remote}\` to read ${branch} — nothing was changed` };
  if (where !== 'present') return { ok: true, source: 'local' };
  const res = transact(root, { remote, branch, retries, message: `registry: activate milestone ${number}` }, (files) => {
    const registry = parseRegistry(files[REGISTRY_FILE]);
    const c = registry.claims.find((x) => x.type === 'milestone' && x.number === number);
    if (!c) return { updates: {}, result: { error: `milestone ${number} is not claimed — declare it with \`ac milestone new --planned\`` } };
    if (c.status === 'complete') return { updates: {}, result: { error: `milestone ${number} is closed` } };
    if (c.status === 'active') return { updates: {}, result: { already: true, name: c.name } };
    c.status = 'active';
    return { updates: { [REGISTRY_FILE]: JSON.stringify(registry, null, 2) + '\n' }, result: { name: c.name } };
  });
  if (!res.ok) return { ok: false, error: res.error || 'registry write failed — nothing was changed' };
  if (res.result.error) return { ok: false, error: res.result.error };
  return { ok: true, source: 'remote', branch, name: res.result.name || '', already: !!res.result.already };
}

// #78 — correct a milestone's name. Unlike a phase's, a milestone name derives no slug and
// no path — it is shown in status, the close messages and the registry only — so changing it
// is safe for any status, `complete` included. Only `name` on the claim is written, through
// the same CAS as `activate`; the latest claim under the number is the one renamed, matching
// milestoneClaims(). `matches` is the duplicate-name check `milestone new` runs, advisory.
export function renameMilestone({ root, number, name, remote, branch, retries = 6 }) {
  remote = remote || registryRemote(root);
  branch = branch || registryBranch(root);
  const where = isRepo(root) && hasRemote(remote, root) ? probeBranch(root, { remote, branch }) : 'no-remote';
  if (where === 'unreachable') return { ok: false, error: `cannot reach \`${remote}\` to read ${branch} — nothing was changed` };
  if (where !== 'present') return { ok: false, error: 'milestone names live in the shared registry, and this project has none — nothing to rename' };
  const res = transact(root, { remote, branch, retries, message: `registry: rename milestone ${number}` }, (files) => {
    const registry = parseRegistry(files[REGISTRY_FILE]);
    const c = [...registry.claims].reverse().find((x) => x.type === 'milestone' && x.number === number);
    if (!c) return { updates: {}, result: { error: `milestone ${number} is not claimed — nothing to rename` } };
    if (c.name === name) return { updates: {}, result: { unchanged: true, name } };
    const matches = computeMatches(registry.claims.filter((x) => x !== c), 'milestone', name);
    const previous = c.name || '';
    c.name = name;
    return { updates: { [REGISTRY_FILE]: JSON.stringify(registry, null, 2) + '\n' }, result: { previous, matches } };
  });
  if (!res.ok) return { ok: false, error: res.error || 'registry write failed — nothing was changed' };
  if (res.result.error) return { ok: false, error: res.result.error };
  return { ok: true, branch, ...res.result };
}

// #37 — the milestone claims, by number (latest claim wins on a duplicated number).
export function milestoneClaims(registry) {
  const out = new Map();
  for (const c of (registry && registry.claims) || []) if (c.type === 'milestone') out.set(c.number, c);
  return out;
}

// #76 — milestones claimed and waiting to be started: `planned`, OR `active` but not the
// project's current one. The second shape is every claim made before v0.28.0 and every
// `ac claim milestone` (claim() defaults to active); #37's guidance saw only the first.
export function waitingMilestones(registry, current) {
  return [...milestoneClaims(registry).values()]
    .filter((c) => c.status === 'planned' || (c.status === 'active' && c.number !== current))
