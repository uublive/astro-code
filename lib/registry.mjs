...
// max(EVERY claim of this type [and milestone]) + 1, else 1. Allocation is
// monotonic: completed claims still count, so a number is never reused after its
// milestone/phase is archived (the bug that reset the counter to 1 on completion).
function nextNumber(registry, type, milestone) {
  const nums = registry.claims
    .filter((c) => c.type === type && (type !== 'phase' || c.milestone === milestone))
    .map((c) => c.number);
...
export function claim({ root, type, milestone, name = '', remote, branch, retries = 6 }) {
  ...
  const res = transact(root, { remote, branch, retries, message }, (files, id) => {
    ...
    if (type === 'milestone') {
      ...
      // add` and number from 1 — no phantom auto-reserved phase 1 (which used to
      // push the first real phase to 2 without ever seeding the roadmap).
