...
/** The lifecycle statuses an entry may be in. `merged` is terminal (P3, D6). */
export const PRINCIPLE_STATUSES = ['proposed', 'accepted', 'rejected', 'retired', 'superseded', 'merged'];
...
//
// `sighting` is a repeatable header key, not a body section, for the same reason
// `promotion`/`history` are: one JSON record per line under the existing fixed-order
// strict parse, so a hand-mangled store still reads as damage rather than silently
// losing evidence. Sightings are append-only and deliberately kept OUT of `history` —
// `history` is the lifecycle/revision-marker list `compareRevisions` diffs, and C2
// requires an accepted or rejected entry's history to stay byte-identical after a
// sighting. Keeping them separate is also what makes them sync-mergeable (P4): two
// machines can each append a sighting line and never conflict, because neither side's
// existing lines ever change.
//
// `merged-into` mirrors `superseded-by` (its own key, not an overload of it) because
// `compareRevisions` and sync need to tell a supersession from a merge apart, and C7
// requires the duplicate to still cite a specific survivor id.
//
// Order is now enforced by the parser below (see `parsePrinciple`) — ADR-059 always
// said "fixed header key order", but earlier the parser only assembled a header of
// unknown-order lines that happened to render canonically; nothing rejected a
// hand-mangled order as damage, so C9's "read a mangled header as damage" case had no
// enforcement to point at.
// Exported (phase 27 t8) so the read-contract doc's guard test can assert the fixed
// header order it publishes to a foreign reader (`templates/PRINCIPLES-CONTRACT.md`)
// matches this module's actual order byte-for-byte — the one place that order is
// authoritative.
export const HEADER_KEYS = [
  'id', 'kind', 'strength', 'status', 'created', 'stack', 'work',
  'files', 'reason', 'superseded-by', 'merged-into', 'source', 'promotion', 'history', 'sighting',
];
const REQUIRED_KEYS = ['id', 'kind', 'strength', 'status', 'created'];
const REPEATABLE_KEYS = new Set(['files', 'promotion', 'history', 'sighting']);
...
export function parsePrinciple(text, { file, id: expectedId } = {}) {
  ...
  for (const line of headerLines) {
    ...
    const key = m[1];
    const value = m[2] || '';
    const keyIdx = HEADER_KEYS.indexOf(key);
    if (keyIdx === -1) throw damaged(label, `unknown header key ${JSON.stringify(key)}`);
    if (keyIdx < lastKeyIdx) {
      throw damaged(label, `header key ${JSON.stringify(key)} out of order (fixed order: ${HEADER_KEYS.join(', ')})`);
    }
    lastKeyIdx = keyIdx;
    ...
    if (key === 'sighting') { sightingRaw.push(value); continue; }
  ...
  const mergedInto = single.has('merged-into') ? single.get('merged-into') : undefined;
  ...
  if (status === 'merged' && !mergedInto) throw damaged(label, 'status merged requires merged-into');
  if (status !== 'merged' && mergedInto !== undefined) {
    throw damaged(label, `merged-into present but status is ${status}`);
  }
  ...
  const sightings = sightingRaw.map((v) => parseJSONObject(label, 'sighting', v));
  ...
  if (mergedInto !== undefined) entry.mergedInto = mergedInto;
  if (source !== undefined) entry.source = source;
  if (sightings.length) entry.sightings = sightings;
  return entry;
}
...
export function renderPrinciple(entry) {
  const {
    id, kind, strength, status, created,
    scopes = {}, reason, supersededBy, mergedInto, source,
    promotions = [], history = [], sightings = [],
    statement, why,
  } = entry;
  ...
  if (status === 'merged') lines.push(`merged-into: ${mergedInto}`);
  ...
  for (const s of sightings) lines.push(`sighting: ${JSON.stringify(s)}`);
  lines.push('---');
  lines.push('');
...
/**
 * The union of two sighting arrays, deduplicated by `JSON.stringify` equality, sorted
 * by (`at` ascending, then the JSON string) so both machines produce a byte-identical
 * file regardless of which side proposed which order (P4, D3).
 *
 * @param {object[]} a
 * @param {object[]} b
 * @returns {object[]}
 */
export function unionSightings(a = [], b = []) {
  const seen = new Map(); // JSON string -> record
  for (const s of [...a, ...b]) seen.set(JSON.stringify(s), s);
...
/**
 * Reconcile two revisions of the same entry for sync (P4, D3, C8). Sightings are
 * append-only evidence with no "winner" — there is nothing to conflict over — so they
 * are stripped before the UNCHANGED `compareRevisions` runs on the lifecycle fields,
 * then unioned back in regardless of which side's lifecycle won. A genuine conflict
 * (divergent history) is untouched: it is reported exactly as before.
 *
 * @param {object} a
 * @param {object} b
 * @returns {{ outcome: 'same'|'older'|'newer' }|{ outcome: 'same'|'older'|'newer', entry: object }|{ outcome: 'conflict' }}
 */
export function reconcileRevisions(a, b) {
  const { sightings: sa, ...aBare } = a;
  const { sightings: sb, ...bBare } = b;
  const outcome = compareRevisions(aBare, bBare);
  if (outcome === 'conflict') return { outcome: 'conflict' };

  const base = outcome === 'older' ? b : a;
  const union = unionSightings(sa, sb);
  const entry = { ...base };
  if (union.length) entry.sightings = union;
  else delete entry.sightings;
