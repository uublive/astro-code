...
export function classifyClaudeLine(obj) {
  ...
  if (typeof content === 'string') {
    ...
    // Every block must be a `text` block carrying a STRING `text` (C9): `b?.text || ''`
    // used to read a drifted block (a number, an object, a missing field) as an empty
    // turn, so the drift vanished instead of being counted.
    if (content.length && !content.every((b) => b?.type === 'text' && typeof b.text === 'string')) return { kind: 'unrecognised' };
