...
export function renderCommand(name, source) {
  ...
  return [file, sidecar(skillName, description, { implicit: true })];
}
...
export function renderAgent(name, source) {
  const { skillName, description, file } = skillBody(name, source);
  return [file, sidecar(skillName, description, { implicit: false })];
}

/**
 * The agents/openai.yaml sidecar, per skill-creator's own reference doc
 * (references/openai_yaml.md on any Codex install).
 *
 * Two documented constraints that are easy to violate silently:
 *   - short_description is a 25–64 char UI blurb, so it is clamped, not just
 *     whatever the first sentence happens to be
 *   - default_prompt MUST name the skill as `$skill-name` — that is how a user
 *     invokes one, and it is the string the UI inserts
 *
 * `allow_implicit_invocation` is the field that decides whether the model may
 * reach for the skill on its own. Commands want true: discovery from the
 * description is the whole point. Agents want false: an executor is dispatched
 * deliberately as a step in a wave, never picked up mid-conversation.
 */
function sidecar(skillName, description, { implicit }) {
  return {
    path: `${skillName}/agents/openai.yaml`,
    content: toYaml({
      interface: {
        display_name: skillName,
        short_description: clampBlurb(description, skillName),
...
/** Fit a description into the documented 25–64 character UI blurb. */
function clampBlurb(description, fallbackName) {
  const first = (description.split(/(?<=\.)\s/)[0] || description).trim();
  const base = first || `The ${fallbackName} skill`;
  // Pad a terse description up to the documented floor rather than emitting a
  // blurb Codex considers out of range.
  if (base.length < 25) return `${base} — astro-code workflow`.slice(0, 64);
  if (base.length <= 64) return base;
  // Cut on a word boundary so the blurb stays readable rather than sliced.
  const cut = base.slice(0, 63);
  const space = cut.lastIndexOf(' ');
  return `${(space > 25 ? cut.slice(0, space) : cut).trimEnd()}…`;
}
 ...
 */
export function execCommand({ prompt, model, cwd, worktree, schemaFile, outFile, sandbox, json, reasoning } = {}) {
  const args = ['exec'];
  if (model) args.push('--model', model);
  // Codex takes reasoning depth through the generic config override rather than
  // a dedicated flag. Values: none | low | medium | high | xhigh.
...
export const capabilities = { worktree: true, outputSchema: true, reasoning: true };
