...
import { hostReasoning } from '../reasoning.mjs';
...
export function buildInvocation(host, task, { workdir } = {}) {
  ...
  const { command, args } = host.execCommand({
    ...
    reasoning: task.reasoning ? hostReasoning(task.reasoning, host.id) : undefined,
    json: task.json,
  });
  ...
  return {
    ...
    // What the host will actually run at, after ceiling-clamping. Reported so a
    // caller asking for `max` on a host whose ceiling is lower can see that,
    // rather than assuming it was honoured.
    reasoning: task.reasoning ? hostReasoning(task.reasoning, host.id) : null,
    reasoningApplied: task.reasoning ? Boolean(caps.reasoning) : null,
  };
}
