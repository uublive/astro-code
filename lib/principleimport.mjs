...
export function planForgeImport(entries, doc, { now, idFor }) {
  ...
  for (const node of doc.nodes) {
    ...
    if (known) {
      // A known entry may itself have been merged away since it was keyed — the
      // sighting still has to land on the SURVIVOR (P4 step 1 reuses the same
      // mergedInto-following rule step 2 uses for a fresh statement match).
      const target = resolveSurvivor(entries, known);
      const existingKeys = new Set((target.sightings ?? []).map((s) => s.ref));
      const newSignals = node.signals.filter((sig) => !existingKeys.has(`${forgeRef}#${forgeSignalKey(slug, sig)}`));
      if (newSignals.length) {
        sighted.push({
          id: target.id, slug, added: newSignals.length,
          sightings: newSignals.map((sig) => signalToSighting(slug, sig, node.created ?? doc.exported_at)),
        });
      } else {
        unchanged.push(slug);
      }
      slugToId.set(slug, target.id);
      continue;
    }
