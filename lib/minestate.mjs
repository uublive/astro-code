...
// the watermark past a turn the agent meant to keep.
//
// Only the agent's `keep` carry-over is bounded (`KEPT_MAX`, oldest dropped first), so a
// cold one-off kept sweep after sweep cannot make the file grow forever. Budget-HELD
// turns are never evicted: they are material the user has not seen yet, and they drain
// by themselves at `MINE_BATCH` per sweep. A single shared bound (the old `PENDING_MAX =
// 500`) evicted held overflow, so a 1000-turn backlog only ever handed over 650 turns and
// the rest were lost for good (C7, remediate-r2). The held list is bounded in practice by
// the transcript material itself — one pointer entry per unseen turn.
//
// ## Why `seen` is gone
//
...
/** Most `kept` carry-over entries (the agent's --keep); held entries are never bounded. */
export const KEPT_MAX = 500;
...
/**
 * Keep every `held` entry and the newest `max` `kept` entries (by `at`; entries with no
 * `at` count as oldest), preserving the input order — the sweep hands carried turns over
 * in this order, so reordering here would starve the oldest held-back turns.
 */
function boundKept(pending, max = KEPT_MAX) {
  const kept = pending.map((p, i) => ({ p, i })).filter(({ p }) => p.reason !== 'held');
  if (kept.length <= max) return pending;
  ...
  const drop = new Set(order.slice(0, kept.length - max).map((o) => o.i));
...
export async function advance(storeDir, id, { keep = [] } = {}) {
  ...
  return withLock(join(mineDir(storeDir), '.lock'), () => {
    ...
    const newPending = boundKept([...outOfScope, ...(run.held || []), ...kept]);
